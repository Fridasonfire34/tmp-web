...
export default function SequencesAddPage() {
  ...
  const onSubmit: SubmitHandler<Inputs> = data => {
    ...
    axios('/api/sequences/add', {
      ...
      .then(() => {
        toast.success('Secuencia añadida correctamente');
        router.back();
      })
      .catch(() => {
        toast.error('Ha ocurrido un error al añadir la secuencia');
      })
      .finally(() => {
        setLoading(false);
      });
  ...
  return (
    <Layout>
      ...
      <Typography variant="body1" gutterBottom>
        En esta sección se añade una nueva secuencia.
      </Typography>
      <Stack direction="column" my={3}>
        <form onSubmit={handleSubmit(onSubmit)}>
          <Card>
            <CardHeader title="Datos de la secuencia" />
            <CardContent
            ...
            >
              ...
              <TextField
                ...
                label="Scanned By"
                {...register('scannedBy', {
                  required: 'Scanned By es requerido'
                })}
                error={!!errors.scannedBy}
                helperText={errors.scannedBy?.message}
              />
            </CardContent>
            <CardActions>
              <Button
